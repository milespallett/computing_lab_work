<?php
  ...
  if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if(empty($email_err) && empty($password_err)){
      ...
      // Prepare query
      $sql = "SELECT email, firstname, pword FROM Student WHERE email = :studentid AND pword = :password";

      // Prepare statement
      if($stmt = $pdo->prepare($sql)){
        // Bind params
        $stmt->bindParam(':studentid', $sanitizedEmail, PDO::PARAM_STR);
        ...
        if($stmt->execute()){
          ...
          if($stmt->rowCount() === 1){
                ...
                $password_err = 'Email address or password was incorrect';
              }
            //}
        } else {
